...
struct UniformBlock
{
	...
	float mAnimationSpeed = 0.01f;
	float mAmplitude = 0.4f;
	float mFrequency = 2.0f;

	// Point Light Information
	vec4 mLightPositionTime;
	vec3 mLightColor;
};
...
float				gAnimationSpeedOverride = 0.01f;
float				gAmplitudeOverride = 0.4f;
float				gFrequencyOverride = 2.0f;
...
public:
	bool Init()
	{
		...
		pGuiWindowMaterial = gAppUI.AddGuiComponent("Wobbling Properties", &guiDesc);
		pGuiWindowMaterial->AddWidget(SliderFloatWidget("Animation Speed", &gAnimationSpeedOverride, 0.001f, 0.04f));
		pGuiWindowMaterial->AddWidget(SliderFloatWidget("Amplitude", &gAmplitudeOverride, 0.0f, 1.0f));
		pGuiWindowMaterial->AddWidget(SliderFloatWidget("Frequency", &gFrequencyOverride, 1.0f, 5.0f));
	...
	void Update(float deltaTime)
	{
		...
		gUniformData.mLightPositionTime = vec4(0, 0, 0, currentTime);
		gUniformData.mLightColor = vec3(0.9f, 0.9f, 0.7f);    // Pale Yellow
		gUniformData.mAnimationSpeed = gAnimationSpeedOverride;
		gUniformData.mAmplitude = gAmplitudeOverride;
		gUniformData.mFrequency = gFrequencyOverride;
